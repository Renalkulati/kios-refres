...
import { createOrder, decreaseStock, validateVoucher, updateOrderStatus, cancelOrder } from "../../lib/db.js";
...
export function Splash({ onDone, settings }) {
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "linear-gradient(135deg, var(--c-primary-dark,#1E3A8A) 0%, var(--c-primary,#2563EB) 100%)", padding: 24 }}>
      ...
      <h1 style={{ color: "#fff", fontWeight: 900, fontSize: 32, letterSpacing: "-0.5px", marginBottom: 6 }}>{settings?.store_name || "KIOS REFRES"}</h1>
...
<span style={{ color: "#fff", fontWeight: 900, fontSize: 17 }} className="hide-mobile">{settings?.store_name || "KIOS REFRES"}</span>
          ...
          <div style={{ flex: 1, position: "relative" }}>
            ...
            <div style={{ position: "relative", display: "flex", flexDirection: "column", gap: 6 }}>
              <label style={{ color: "rgba(255,255,255,.8)", fontSize: 11, fontWeight: 700 }}>🔍 Cari Produk</label>
              <input value={q} onChange={e => setQ(e.target.value)} placeholder="Cari nama produk, kategori..." style={{ width: "100%", padding: "12px 14px 12px 44px", borderRadius: 99, border: "1.5px solid rgba(255,255,255,.25)", background: "rgba(255,255,255,.12)", color: "#fff", fontSize: 14, outline: "none", backdropFilter: "blur(8px)", boxSizing: "border-box", fontWeight: 500 }} />
            </div>
    ...
    return (
      <div style={{ maxWidth: 560, margin: "0 auto", padding: "0 16px 100px" }}>
        ...
        <div style={{ background: "#fff", borderRadius: 18, overflow: "hidden", boxShadow: "0 2px 12px rgba(0,0,0,.08)", border: "1px solid #F1F5F9" }}>
          ...
          {(selected.order_status === "diproses" || selected.order_status === "menunggu_bayar" || selected.order_status === "menunggu_konfirmasi") && !claimedIds[selected.order_id] && (
            <div style={{ padding: "14px 18px", borderTop: "1px solid #F1F5F9" }}>
              <button onClick={() => claimPaid(selected.order_id)} disabled={claiming} style={{ width: "100%", padding: "12px", borderRadius: 12, background: "#D1FAE5", border: "1.5px solid #6EE7B7", fontWeight: 800, fontSize: 14, color: "#065F46", cursor: "pointer", marginBottom: 8 }}>{claiming ? "⏳ Memproses…" : "✅ Saya Sudah Bayar"}</button>
            </div>
          )}
          {claimedIds[selected.order_id] && <div style={{ padding: "12px 18px", background: "#F0FDF4", textAlign: "center", fontWeight: 800, fontSize: 13, color: "#059669", marginBottom: 8 }}>✅ Dikonfirmasi — sedang diverifikasi penjual</div>}
          {["diproses", "menunggu_bayar", "menunggu_konfirmasi"].includes(selected.order_status) && (
            <div style={{ padding: "12px 18px", borderTop: "1px solid #F1F5F9", background: "#FEF2F2" }}>
              <button onClick={async () => {
                if (!confirm("Yakin batalkan pesanan ini?")) return;
                try {
                  await cancelOrder(selected.order_id);
                  toast?.add?.("✅ Pesanan dibatalkan");
                  setSelected(null);
                } catch (e) {
                  alert("Gagal: " + e.message);
                }
              }} style={{ width: "100%", padding: "12px", borderRadius: 12, background: "#FEE2E2", border: "1.5px solid #FECACA", fontWeight: 800, fontSize: 14, color: "#DC2626", cursor: "pointer" }}>
                ✕ Batalkan Pesanan
              </button>
            </div>
          )}
