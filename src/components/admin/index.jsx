...
function ReceiptPrint({ order, onClose }) {
  ...
  const doPrint = () => {
    ...
    sty.id = "rp_style";
    sty.innerHTML = `@media print{body>*:not(#rp_root){display:none!important}#rp_root{display:block!important;position:fixed;inset:0;z-index:99999;background:#fff;font-family:"Courier New",monospace;font-size:11px;line-height:1.3}.rp_noprint{display:none!important}@page{margin:1mm 2mm;size:80mm 200mm}}#rp_root .receipt{width:72mm;margin:4mm auto;padding:8px 6px;background:#fff;border:1px solid #ccc;border-radius:4px;box-shadow:none}`;
  ...
  return (
    <div id="rp_root" style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.6)", zIndex: 600, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ background: "#fff", borderRadius: 18, padding: 22, maxWidth: 380, width: "100%", maxHeight: "92vh", overflowY: "auto", boxShadow: "0 24px 80px rgba(0,0,0,.3)" }}>
        ...
        <div className="receipt" style={{ fontFamily: "'Courier New',monospace", fontSize: 11, padding: "12px 8px", border: "1px dashed #ccc", borderRadius: 4, lineHeight: 1.2 }}>
          <div style={{ textAlign: "center", paddingBottom: 8, marginBottom: 8, borderBottom: "2px dashed #ccc" }}>
            <p style={{ fontWeight: 900, fontSize: 14, letterSpacing: 1, margin: 0 }}>★ KIOS REFRES ★</p>
            <p style={{ fontSize: 9, color: "#666", margin: "2px 0" }}>Struk Pembelian</p>
            <p style={{ fontSize: 9, color: "#666", margin: 0 }}>{fmtDate(order.order_date)}</p>
          </div>
